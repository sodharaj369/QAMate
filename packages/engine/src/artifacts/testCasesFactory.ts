...
export class TestCasesFactory {
  public static generateCases(context: GeneratorContext, type: string, persona: string): string {
    ...
    const isSecurityDomain =
      domains.includes('Authentication') || domains.includes('Payments') || domains.includes('API');
    const isApiOrDb =
    ...
    if (rules.length > 0) {
      ...
      sections.push(
        `- [ ] **TC-POS-1**: Verify standard functional path execution of "${context.requirement.title}".`,
      );
    }
    ...
    sections.push(
      `- [ ] **TC-NEG-1**: Trigger validation errors by submitting empty or malformed inputs.`,
    );
    sections.push(
      `- [ ] **TC-NEG-2**: Verify authentication denial / authorization lockout for unauthorized actions.`,
    );

    // 3. Boundary & Edge Case Checklist
    sections.push(`\n#### 📐 Boundary & Edge Case Checklist`);
    sections.push(
      `- [ ] **TC-BND-1**: Assert character limit overflows (e.g., input values at limit, limit + 1).`,
    );
    sections.push(
      `- [ ] **TC-EDGE-1**: Trigger race conditions or concurrent connection interrupts.`,
    );
    ...
    if (isApiOrDb) {
      ...
      sections.push(
        `- [ ] **TC-API-1**: Verify JSON payload compliance, schema structure, and header validations.`,
      );
      sections.push(
        `- [ ] **TC-DB-1**: Confirm database persistence commits clean records and does not log confidential parameters.`,
      );
    }
    ...
    if (isSecurityDomain) {
      ...
      sections.push(
        `- [ ] **SEC-1**: Verify token injection exploits, signature tampering, and session expiry lockdowns.`,
      );
      sections.push(
        `- [ ] **SEC-2**: Audit response headers for credential leakage (e.g., masking secrets, avoiding plain text API keys).`,
      );
    }
    ...
    const hasPerformanceTrigger =
      ...
      contentLower.includes('instant');
    if (hasPerformanceTrigger) {
      sections.push(`\n#### ⚡ Performance & Resource SLA Checklist`);
      sections.push(
        `- [ ] **PERF-1**: Assert API responses execute under 200ms SLA threshold guidelines.`,
      );
      sections.push(
        `- [ ] **PERF-2**: Run stress simulation asserting connection stability under peak request volume.`,
      );
    }
    ...
    if (isUIFeature) {
      ...
      sections.push(
        `- [ ] **ACC-1**: Verify keyboard navigation (Tab indices) and focus indicator visibility.`,
      );
      sections.push(
        `- [ ] **ACC-2**: Assert screen reader accessibility via ARIA labels and semantic markup validation.`,
      );
    }
